    ...
    while (TRUE) {
        ...
        if (0 < (recv(client_socket, message, sizeof(message), 0))) { // recv 1
            ...
            case DEVICE:
                ...
                send(client_socket, "received", strlen("received"), 0);//송신
...
void detector() {
    ...
    while (true) {
        ...
        // if (cvui::button(frame, 600, 2.5, " &X")) break;
        ...
        if (cvui::mouse(cvui::CLICK)) {
            // 클릭 이벤트
            //cvui::text(frame, 20, 660, "Mouse was clicked!");
        }
        cvui::rect(frame, rectangle.x, rectangle.y, rectangle.width, rectangle.height, 0xff0000); ///rectangle 테두리 색 바꾸기
        */
        cvui::update();
                    ...
                    if (objectId != -1) {
                        ...
                        if (is_selected) {
                            ...
                            if (selectFunc(cObjectId)) {
                                ...
                                printf("select에서 Error 발생\n");
                        }
                    }
                }
                if (gesture == ON&&is_selected){
                    printf("ON, %s\n", selected_thing_id);
                    if(!statusChangeFuc(selected_thing_id,"on"))printf("statusChange에서 Error 발생\n");
                }
                if (gesture == OFF&&is_selected){
                    printf("OFF, %s\n", selected_thing_id);
                    if (!statusChangeFuc(selected_thing_id, "off"))printf("statusChange에서 Error 발생\n");
                }
            }
